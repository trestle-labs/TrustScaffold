...
export const wizardRuleMatrix: WizardRule[] = [
  ...
  {
    id: 'welcome-website-privacy-notice-warning',
    kind: 'warning',
    step: 'welcome',
    title: 'Website collects personal data without a privacy notice',
    summary: 'If the public website collects personal data, the wizard should confirm that a public privacy notice exists and matches actual processing.',
    criteria: ['P1.1', 'GDPR', 'CCPA'],
    severity: 'warning',
    recommendation: 'Publish or update the privacy notice before relying on generated privacy, GDPR, or CCPA documentation.',
    anchorField: 'company.websiteHasPrivacyNotice',
    when: (data) => data.company.hasPublicWebsite && data.company.websiteCollectsPersonalData && !data.company.websiteHasPrivacyNotice,
  },
  {
    id: 'welcome-cookie-consent-warning',
    kind: 'warning',
    step: 'welcome',
    title: 'Cookies or analytics without consent tooling',
    summary: 'Tracking technologies can create GDPR/UK GDPR, ePrivacy, CCPA/CPRA, and contractual privacy obligations.',
    criteria: ['GDPR', 'CCPA'],
    severity: 'warning',
    recommendation: 'Document the cookie banner, preference center, opt-out link, or other consent/choice mechanism used on the website.',
    anchorField: 'company.websiteHasCookieBanner',
    when: (data) => data.company.hasPublicWebsite && data.company.websiteUsesCookiesAnalytics && !data.company.websiteHasCookieBanner,
  },
  {
    id: 'welcome-privacy-tsc-recommendation',
    kind: 'recommendation',
    step: 'tsc-selection',
    title: 'Website answers indicate privacy scope',
    summary: 'Website collection, EU/UK exposure, California exposure, sale/share, or children\'s data can make privacy controls audit-relevant.',
    criteria: ['P1.1', 'P2.1', 'P5.1', 'GDPR', 'CCPA'],
    recommendation: 'Consider selecting the Privacy TSC so the generated pack includes the Privacy Notice, consent, DSAR, disclosure, data-quality, and monitoring control language.',
    anchorField: 'tscSelections.privacy',
    when: (data) => data.company.hasPublicWebsite && !data.tscSelections.privacy && (data.company.websiteCollectsPersonalData || data.company.websiteTargetsEuOrUkResidents || data.company.websiteTargetsCaliforniaResidents || data.company.websiteSellsOrSharesPersonalInformation || data.company.websiteAllowsChildrenUnder13),
  },
  {
    id: 'welcome-public-website-url-warning',
    kind: 'warning',
    step: 'welcome',
    title: 'Public website selected without URL',
    summary: 'When a public website is in scope, the URL should be captured so generated documents can reference customer-facing processing channels.',
    criteria: ['P1.1', 'CC2.1'],
    severity: 'warning',
    recommendation: 'Add the canonical public website URL so reviewer-facing privacy and communication language is complete.',
    anchorField: 'company.website',
    when: (data) => data.company.hasPublicWebsite && !data.company.website.trim(),
  },
  {
    id: 'services-company-vendor-depth-recommendation',
    kind: 'recommendation',
    step: 'operations',
    title: 'Services profile should emphasize vendor and evidence workflows',
    summary: 'Service-led organizations usually rely on subcontractors, ticketing trails, and customer commitments that need stronger vendor and operational evidence language.',
    criteria: ['CC3.2', 'CC9.2', 'CC2.3'],
    recommendation: 'For services-heavy delivery, ensure subprocessors, review cadence, customer contracts, support channels, and evidence retention are fully documented before generation.',
    anchorField: 'company.businessModel',
    when: (data) => data.company.businessModel === 'services' || data.company.deliveryModel === 'managed-services' || data.company.deliveryModel === 'professional-services',
  },
  {
    id: 'software-company-sdlc-depth-recommendation',
    kind: 'recommendation',
    step: 'security-assessment',
    title: 'Software profile should emphasize SDLC and release controls',
    summary: 'Software-centric delivery generally increases expectations around secure SDLC, branch protections, validation logic, and change evidence.',
    criteria: ['CC5.2', 'CC7.1', 'CC8.1', 'PI1.1'],
    recommendation: 'For software-centric delivery, complete the security assessment and operations sections with concrete SDLC, change-management, and testing evidence details.',
    anchorField: 'company.businessModel',
    when: (data) => data.company.businessModel === 'software' || data.company.deliveryModel === 'saas' || data.company.deliveryModel === 'api-platform' || data.company.deliveryModel === 'self-hosted-product',
  },
  {
